      ...
      <div class="mx-auto">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
          <form action="" method="post">
            <input type="hidden" name="orderIDStatus" value="<p echo $row['orderID'] ?>">
...
// Check if the "Sort by  user name" button is clicked
if (isset($_POST['sort-order-by-name'])) {
    $query = $pdo->query("SELECT * FROM final_order INNER JOIN users ON orders.userID = users.userID INNER JOIN  products ON orders.productID =products.productID ORDER BY users.firstName ;");
    $result = $query->fetchAll(PDO::FETCH_ASSOC);
}else if (isset($_POST['sort-order-by-date'])) {
// Check if the "Sort by  user name" button is clicked
    $query = $pdo->query("SELECT * FROM orders INNER JOIN users ON orders.userID = users.userID INNER JOIN  products ON orders.productID =products.productID  ORDER BY orders.orderDate;");
    ...
    FROM users 
    INNER JOIN final_order ON final_order.user_id = users.userID;");
    $result = $query->fetchAll(PDO::FETCH_ASSOC);
}
                        foreach($result as $row){
                        ?>
